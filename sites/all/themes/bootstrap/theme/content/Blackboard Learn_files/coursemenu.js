 ...
 */
      ...
      if ( result.success == "true" )
      {
        if (confirm(result.removalMessage))
        {
          var removeTocUrl = appendQueryStringFunc( result.actionUrl, "method=removeToc&course_id=" + course_id +
                                                    "&toc_id=" + toc_id + '&retUrl=' + retUrl + '&' + courseMenu.nonceKey + '=' + courseMenu.nonceValue );
  ...
  submitRequestAsPost: function( action, toc_id )
  {
    ...
    var url = this.appendMenuActionUrl( "method=" + action + "&course_id=" + window.course_id + "&toc_id=" + toc_id + "&retUrl=" + escape( document.location.href ) );
