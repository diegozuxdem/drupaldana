            ...
            try
             {
                 ...
                 {
                   var textString = page.bundle.getString("list.ajax.mode.marquee.text");
                   var startString = page.bundle.getString("list.ajax.mode.start.marquee.text") + id+ textString;
                   var endString = page.bundle.getString("list.ajax.mode.end.marquee.text") + id+ textString;
...
inventoryList.PagingController.prototype = 
{
    initialize: function( inventoryListContainer, initPagingValue, initPageIndex, inAjax, limitMaxNumOfItems, pageResultsMaxSize, totalResultsCount )
    {
      ...
      if(!limitMaxNumOfItems)
      {
        ...
        if ( $(this.containerId + '_jumptopage_top') && $(this.containerId + '_jumptopage_top') )
        {
          this.jumpToPage = {
           top: $(this.containerId + '_jumptopage_top').show().remove(),
           bot: $(this.containerId + '_jumptopage_bot').show().remove()
          };
        }        
...
list.CheckboxRadioController.prototype = 
{
    initialize: function( listContainer, hasSelectAll, isSelectAllFromList, listener )
    {
        ...
        var tableHeads = this.container.getElementsByTagName("thead");
